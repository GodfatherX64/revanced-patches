...
import app.revanced.patches.youtube.layout.sponsorblock.signatures.*
import app.revanced.patches.youtube.layout.sponsorblock.utils.instructions.InstructionUtils.findInstructionsByName
import app.revanced.patches.youtube.layout.sponsorblock.utils.signature.resolver.SignatureResolverHelper
...
class SponsorBlockPatch : BytecodePatch(
    listOf(
        PlayerControllerSetTimeReferenceSignature,
        CreateVideoPlayerSeekbarSignature,
        VideoIdSignature,
        VideoTimeSignature,
    )
) {
    ...
    override fun execute(data: BytecodeData): PatchResult {
        ...
        */
        val referenceResult = PlayerControllerSetTimeReferenceSignature.result!!
        val playerControllerSetTimeMethod =
            data.toMethodWalker(referenceResult.immutableMethod).nextMethod(referenceResult.scanResult.startIndex, true)
         ...
         */
        val toStringResult = VideoTimeSignature.result!!
        val constructor = SignatureResolverHelper(toStringResult.definingClassProxy).resolve(object :
            MethodSignature("V", null, listOf("J", "J", "J", "J", "I", "L"), null) {})!!.method
         ...
         */
        val videoIdMethodResult =
            SignatureResolverHelper(data.proxy(data.classes.find { it.type.endsWith("PlaybackLifecycleMonitor;") }!!)).resolve(
                VideoIdSignature
            )!!
        val videoIdMethod = videoIdMethodResult.method
        val videoIdRegister =
            (videoIdMethod.implementation!!.instructions[videoIdMethodResult.scanResult.endIndex + 1] as Instruction11x).registerA

        videoIdMethod.addInstructions(
            videoIdMethodResult.scanResult.endIndex + 2, // after the move result
            """
        ...
        /*
         Seekbar drawing
         */
        val seekbarSignatureResult = CreateVideoPlayerSeekbarSignature.result!!
        val seekbarMethod = seekbarSignatureResult.method
        ...
        */
        ...
        Set video length
         */
        val videoLengthMethodResult =
            SignatureResolverHelper(seekbarSignatureResult.definingClassProxy).resolve(VideoLengthSignature)!!
        val videoLengthMethod = videoLengthMethodResult.method
        val videoLengthMethodInstructions = videoLengthMethod.implementation!!.instructions

        val videoLengthRegister =
            (videoLengthMethodInstructions[videoLengthMethodResult.scanResult.endIndex - 2] as OneRegisterInstruction).registerA
        videoLengthMethod.addInstruction(
            videoLengthMethodResult.scanResult.endIndex,
         ...
         */
        val controlsMethodResult =
            SignatureResolverHelper(data.proxy(data.classes.find { it.type.endsWith("YouTubeControlsOverlay;") }!!)).resolve(
                ShowPlayerControlsSignature
            )!!
        ...
        methods@ for (method in controlsMethodResult.definingClassProxy.resolve().methods) {
        ...
        // change visibility of the buttons
        controlsMethodResult.method.addInstructions(
            0, """
